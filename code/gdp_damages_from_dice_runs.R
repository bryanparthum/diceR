...
data = read_csv('gdp_loss.csv') %>% mutate(temp=round(temp,1),
                                           gdp_loss=round(gdp_loss,3)) %>% filter(temp<8) %>% group_by(damage_fun) %>% slice(1:which.max(temp==max(temp)))

data %<>% group_by(damage_fun) %>% mutate(labels=case_when(temp==max(temp)~damage_fun,TRUE~''))
...
data %>% 
  ...
  scale_color_manual(values=sample(wes_palette(name="BottleRocket1"),10,replace=T)) +
  scale_fill_manual(values=sample(wes_palette(name="BottleRocket1"),10,replace=T)) +
  scale_x_continuous(breaks=c(0,1,2,3,4,5), limits=c(0,10)) +
