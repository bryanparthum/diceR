...
run_dice = function(perturbation_year=-1) {
  ...
  for (t in 1:time_horizon) {
    ...
    # ypc_t0[t] = ygross_orig[t]/(l[t]*1e6)
    
    ypc_growth[t] = 100*((((((al[t]*(l[t]/1000)^(1-gama))*(k[t]^gama))/ (l[t]*1e6)) / (((al[t-1]*(l[t-1]/1000)^(1-gama))*(k[t-1]^gama))/ (l[t-1]*1e6)))) ^(1/5)-1) * (1-(exp(.0127*tatm[t] - 0.0005*tatm[t]^2)-1)) 
    
    ypc[t] = ypc[t-1]*ypc_growth[t]
